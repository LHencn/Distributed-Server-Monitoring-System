/*************************************************************************
	...
	> Created Time: 2018年11月24日 星期六 11时01分52秒
 ************************************************************************/
...
#include "common.h"
   ...
   *socket(int domain, int type, int protocol);
    domain    参数指定那个协议用在当前套接字上；
        AF_INET  IPv1网络协议，TCP和UDP都可以用的协议
        AF_INET3 IPv6网络协议，TCP和UDP都可以用的协议
    ...
    type      用与选择套接字使用的类型
        ...
        SOCK_RAW  提供读取原始的网络协议。这种特殊的套接字可用于手工构建任意类型的协议。一般使用这个套接字来实现ICMP请求（例如ping）
        SOCK_RDM 提供一个可靠的数据层，但不保证到达顺序。一般的操作系统都为实现此功能
...
int socket_create(int port) {
    ...
    /*
     * 服务器端要用bind()函数将套接字与特定的IP地址和端口号绑定起来，只有这样，流经该IP地址和端口的数据才能交给套接字处理；而客户端要用connect()函数建立连接。
     */
    ...
    /*
     *(1)listen()函数的主要作用就是将套接字（sock_fd)变成被动的连接监听套接字（被动等待客户端的连接),至于参数backlog的作用是设置内核中连接队列的长度
    （2）这里需要注意的是，listen()函数不会阻塞，它主要做的事情为，将该套接字和套接字对应的连接队列长度告诉linux内核，然后，listen()函数就结束了。
     *
     */
    return socket_fd;
}
     ...
     * sockaddr 在头文件#include <sys/socketh>中定义，sockaddr的缺陷是：sa_data把目标地址和端口信息混在一起了；
        ...
        sockaddr_in在头文件#include <netinet/in.h>或#include <arpa/inet.h>中定义，该结构体解决了sockaddr的缺陷，把port和addr分开储存在两个变量中；
    *
    */  
    ...
    int socket_connect(int port, char *host) {
        ...
        bool socket_connect(resource $socket, string $address[, int $port = 0 ])
         参数：
            address 如果参数socket是AF_INET，那么参数address也可以是有效的 IPv6 地址（例如 
    ...
    成功时返回 TRUE， 或者在失败时返回 FALSE。
        ...
        */
        ...
        struct sockaddr_in dest_addr = {0}; //目的地址
